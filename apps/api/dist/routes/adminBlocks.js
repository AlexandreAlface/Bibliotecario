...
r.get("/admin/libraries/:libraryId/blocks", (0, auth_1.requireRole)(auth_1.ROLES.ADMIN), async (req, res, next) => {
    try {
        ...
        res.json(slots.map(s => ({
            ...
            reason: null, // sem coluna no schema
        })));
    }
    catch (e) {
        next(e);
...
r.post("/admin/libraries/:libraryId/blocks", (0, auth_1.requireRole)(auth_1.ROLES.ADMIN), async (req, res, next) => {
    try {
        ...
        const anyLib = await prisma_1.prisma.user.findFirst({
            where: {
        ...
        if (!anyLib)
            return res.status(409).json({ error: "biblioteca sem bibliotecários" });
        const created = await prisma_1.prisma.consultationSlot.create({
            data: {
...
r.delete("/admin/libraries/:libraryId/blocks/:id", (0, auth_1.requireRole)(auth_1.ROLES.ADMIN), async (req, res, next) => {
    try {
        ...
        const belongs = slot.libraryId === libraryId ||
            slot.librarian.userLibraries.some(ul => ul.libraryId === libraryId);
        if (!belongs)
            return res.status(403).json({ error: "sem acesso" });
        await prisma_1.prisma.consultationSlot.delete({ where: { id } });
        res.status(204).end();
    }
    catch (e) {
        next(e);
