...
async function upsertBooksAndLink(libraryId, rows) {
    ...
    // substitui holdings desta biblioteca
    ...
    for (let i = 0; i < items.length; i += IMPORT_BATCH_SIZE) {
        ...
        if (toUpdate.length) {
            ...
            WHERE "isbn" IN (${client_1.Prisma.join(toInvalidate)});
          `;
                }
            }
...
async function cleanupOrphanBooks(opts = {}) {
    ...
    SELECT b."isbn"
    FROM "Book" b
    WHERE NOT EXISTS (SELECT 1 FROM "LibraryBook" lb WHERE lb."bookIsbn" = b."isbn");
  `;
...
r.post("/admin/libraries/:libraryId/books/pipeline", (req, res, next) => {
    ...
    const combined = Array.from(byIsbn.values());
    // 3) Check Beja
    const isbns = combined.map((r) => r.ISBN);
    const bejaFlags = await withConcurrency(isbns, options.concurrency, async (isbn, idx) => {
        if (idx % options.concurrency === 0)
            await sleep(150);
        try {
    ...
    const bejaPresent = bejaFlags.filter(Boolean).length;
    const filtered = combined.filter((_, i) => bejaFlags[i]);
    const filteredOut = combined.length - filtered.length;
    // 4) Enriquecer Lisboa
    const lisboaResults = await withConcurrency(filtered, options.concurrency, async (row, idx) => {
        if (idx % options.concurrency === 0)
            await sleep(150);
    ...
    // 6) Importar
    const { inserted, updated, linked, isbnsToReindex } = await upsertBooksAndLink(libraryId, filtered);
    // 7) Recalcular embeddings (opcional) usando novo fluxo
    let embeddings = null;
    if (recalc && isbnsToReindex.length)
        embeddings = await reindexEmbeddingsForIsbns(isbnsToReindex, options.concurrency ?? 4);
    res.json({
        ok: true,
