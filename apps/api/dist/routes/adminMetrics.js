 ...
 * }
 */
exports.adminMetricsRouter.get("/admin/libraries/:libraryId/metrics", (0, auth_1.requireRole)(auth_1.ROLES.ADMIN, auth_1.ROLES.LIBRARIAN), // admin ou bibliotecário
async (req, res) => {
    ...
    try {
        ...
        const weeklyRows = await prisma_1.prisma.$queryRaw `
      ...
      `;
        ...
        const slotRows = await prisma_1.prisma.$queryRaw `
      ...
      `;
        const slotUtilization = slotRows.map((r) => {
            ...
            const pct = usable > 0 ? Math.round(((Number(r.booked) || 0) / usable) * 1000) / 10 : 0;
            return {
        ...
        const activeLibrarians = await prisma_1.prisma.userLibrary.count({
            where: {
                libraryId,
                user: {
                    userRoles: {
        ...
        const familiesRows = await prisma_1.prisma.$queryRaw `
