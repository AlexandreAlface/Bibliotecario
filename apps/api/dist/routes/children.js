...
router.post("/children", auth_js_1.requireAuth, async (req, res, next) => {
    try {
        ...
        const child = await prisma_js_1.prisma.child.create({
            data: {
                name: name.trim(),
                birthDate: bd,
                gender: gender || null,
...
router.patch("/children/:id", auth_js_1.requireAuth, async (req, res, next) => {
    try {
        ...
        const child = await prisma_js_1.prisma.child.update({ where: { id: childId }, data });
        res.json({
            id: child.id,
            name: child.name,
            birthDate: child.birthDate,
...
router.delete("/children/:id", auth_js_1.requireAuth, async (req, res, next) => {
    try {
        const childId = Number(req.params.id);
        await assertBelongsToFamily(childId, req.user.sub);
        await prisma_js_1.prisma.child.delete({ where: { id: childId } }); // 🎉 sem limpeza manual
        res.status(204).end();
    }
    catch (e) {
        if (e?.status === 404)
