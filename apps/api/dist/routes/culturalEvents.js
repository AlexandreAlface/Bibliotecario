...
exports.culturalEventsRouter.get("/cultural-events", async (req, res) => {
    try {
        ...
        const rangeFilter = toISO && !isNaN(toISO.getTime())
            ? {
                OR: [
        ...
        if (myId) {
            ...
            const isFamily = roleNames.includes(auth_1.ROLES.FAMILY) ||
                roleNames.includes("FAMÍLIA") ||
                roleNames.includes("FAMILIA");
            if (isFamily && rows.length) {
                const ids = rows.map((r) => r.id);
                const resvs = await prisma_1.prisma.eventReservation.findMany({
