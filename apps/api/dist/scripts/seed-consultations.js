...
async function ensureUserWithRole(fullName, email, roleName) {
...
function at(date, h, m = 0) {
    const d = new Date(date);
    d.setHours(h, m, 0, 0);
    return d;
}
async function ensureSlot(librarianId, startAt, endAt, status = client_1.SlotStatus.OPEN) {
...
async function main() {
    ...
    //—— bibliotecários
    const bib1 = await ensureUserWithRole("João Bibliotecário", "joao.biblio@localhost.com", "BIBLIOTECÁRIO");
    const bib2 = await ensureUserWithRole("Carla Bibliotecária", "carla.biblio@localhost.com", "BIBLIOTECÁRIO");
    const bib3 = await ensureUserWithRole("Rita Bibliotecária", "rita.biblio@localhost.com", "BIBLIOTECÁRIO");
    ...
    for (const bib of [bib1, bib2, bib3]) {
        const s1 = await ensureSlot(bib.id, at(tomorrow, 10, 0), at(tomorrow, 10, 30));
        const s2 = await ensureSlot(bib.id, at(tomorrow, 10, 30), at(tomorrow, 11, 0));
        const s3 = await ensureSlot(bib.id, at(tomorrow, 11, 0), at(tomorrow, 11, 30));
