...
function at(d: Date, hour: number, minute = 0) {
  const x = new Date(d);
...
async function getRoleIdByName(name: string) {
  ...
  if (!r) throw new Error(`Role "${name}" não encontrada`);
  return r.id;
}
...
async function seedBadges() {
  const badges = [
    ...
    },
    {
      ...
      type: "STAMP",
      criteria:
        "Ler, pelo menos, um livro de quatro géneros literários diferentes.",
    },
    {
      name: "Pontual",
      type: "STAMP",
    ...
    },
    {
      ...
      type: "STAMP",
      criteria:
        "Explorar cinco conteúdos de boas práticas sugeridos pelo chatbot.",
    },
    { name: "Mini Bibliófilo", type: "TROFÉU", criteria: "Ler 10 livros." },
    {
      ...
      type: "TROFÉU",
      criteria:
        "Todos os elementos da família registados leram pelo menos um livro.",
    },
    {
...
async function seedAgenda(
  ...
  const otherSlot = await prisma.consultationSlot.findFirst({
    where: { librarianId, status: SlotStatus.OPEN, id: { not: firstSlot.id } },
    orderBy: { startAt: "asc" },
  });
  if (otherSlot) {
...
async function main() {
  ...
  const { admin, librarian, family1, family2, ines, tiago, ana } =
    await seedUsersAndFamilies(lib.id);
