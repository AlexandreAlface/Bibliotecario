...
function requireFields<T extends Record<string, any>>(obj: T, keys: (keyof T)[]): void {
  for (const k of keys) if (!obj?.[k]) throw new Error("missing_fields");
}

/** Determina se o utilizador é ADMIN. — Alexandre Brissos — 2025-10-02 */
function isAdmin(me?: { roles?: string[] }): boolean {
...
function normalizeUserBody(body: any) {
  const { fullName, email, phone, password } = body || {};
  return { fullName, email, phone, password } as {
...
async function getOrCreateLibrarianRole(): Promise<Role> {
  const aliases = ["BIBLIOTECÁRIO", "BIBLIOTECARIO", "LIBRARIAN"];
  const found = await prisma.role.findFirst({ where: { name: { in: aliases } } });
  return found ?? prisma.role.create({ data: { name: "BIBLIOTECÁRIO" } });
}
...
async function createUserWithPassword(
  ...
  return prisma.user.create({ data: { fullName, email, phone: phone || null, passwordHash } });
}
...
async function updateUserIfChanged(
  ...
  if (changes.fullName && changes.fullName !== user.fullName) data.fullName = changes.fullName;
  if (changes.phone && changes.phone !== user.phone) data.phone = changes.phone;
...
async function ensureUserRole(userId: number, roleId: number): Promise<void> {
  const has = await prisma.userRole.findUnique({ where: { userId_roleId: { userId, roleId } } });
  if (!has) await prisma.userRole.create({ data: { userId, roleId } });
}

/** Garante ligação user↔library. — Alexandre Brissos — 2025-10-02 */
async function ensureUserLibrary(userId: number, libraryId: number): Promise<void> {
  const has = await prisma.userLibrary.findUnique({ where: { userId_libraryId: { userId, libraryId } } });
  if (!has) await prisma.userLibrary.create({ data: { userId, libraryId } });
}

/** Verifica se o utilizador pode gerir a biblioteca. — Alexandre Brissos — 2025-10-02 */
async function canManageLibrary(me: { id: number; roles: string[] } | undefined, libraryId: number): Promise<boolean> {
...
async function detachUserFromLibrary(userId: number, libraryId: number): Promise<boolean> {
  const link = await prisma.userLibrary.findUnique({ where: { userId_libraryId: { userId, libraryId } } });
  if (!link) return false;
  await prisma.userLibrary.delete({ where: { userId_libraryId: { userId, libraryId } } });
  return true;
}

/** Criação completa de bibliotecário (endpoint admin/librarians). — Alexandre Brissos — 2025-10-02 */
async function createAdminLibrarian(payload: any): Promise<Pick<User, "id" | "fullName" | "email">> {
  ...
  const role = await prisma.role.upsert({ where: { name: ROLES.LIBRARIAN }, update: {}, create: { name: ROLES.LIBRARIAN } });
...
adminLibrariansRouter.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!(await canManageLibrary(me, libraryId))) return res.status(403).json({ error: "forbidden" });
