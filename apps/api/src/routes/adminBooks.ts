...
async function cleanupOrphanBooks(opts: { dryRun?: boolean } = {}) {
  ...
  `;
  const isbns = rows.map((r) => r.isbn);
  if (!isbns.length) return { deleted: 0, candidates: 0, details: {} };
...
r.post(
  ...
  async (req, res) => {
    ...
    const bejaFlags = await withConcurrency(
      isbns,
      options.concurrency!,
      async (isbn, idx) => {
        if (idx % options.concurrency! === 0) await sleep(150);
        try {
    ...
    // 4) Enriquecer Lisboa
    const lisboaResults = await withConcurrency(
      filtered,
      options.concurrency!,
      async (row, idx) => {
        if (idx % options.concurrency! === 0) await sleep(150);
    ...
    filtered.forEach((row, i) => {
      row["Assuntos_Beja"] = row["Assuntos_Beja"]
        ...
        : "Disponível em Beja";
      const meta = lisboaResults[i] as any;
    ...
    if (recalc && isbnsToReindex.length)
      embeddings = await reindexEmbeddingsForIsbns(
        isbnsToReindex,
