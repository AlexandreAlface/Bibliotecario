...
function parseSearchParams(qs: any): SearchParams {
...
function buildWhereBase(p: SearchParams): Prisma.BookWhereInput {
  return {
    AND: [
      p.q
        ? {
...
function mapBookDetail(b: {
  ...
  if (Array.isArray(b.genres)) categories.push(...b.genres.filter(Boolean));
...
const listBasic: RequestHandler = async (req, res, next) => {
  try {
...
const listCurrent: RequestHandler = async (_req, res, next) => {
  try {
...
const listSuggestions: RequestHandler = async (_req, res, next) => {
  try {
...
const searchBooks: RequestHandler = async (req, res, next) => {
  try {
    const p = parseSearchParams(req.query);
    const base = buildWhereBase(p);
    const { items, total } = await searchWithFallback(
      base,
      p.libraryId,
      p.skip,
      p.perPage
...
const getByIsbn: RequestHandler = async (req, res, next) => {
  try {
