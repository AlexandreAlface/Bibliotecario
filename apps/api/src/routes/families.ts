...
r.get(
  ...
  async (req, res) => {
    ...
    if (!roleFamily) return res.json({ items: [], nextCursor: null });
  ...
  async (req, res) => {
    ...
    const [upcomingConsults, recentConsults] = await Promise.all([
      prisma.consultation.findMany({
        where: {
          familyId: id,
          status: { in: [$Enums.ConsultationStatus.PENDING, $Enums.ConsultationStatus.CONFIRMED] },
          startAt: { gte: now },
        },
        orderBy: { startAt: "asc" },
        take: 8,
      ...
      prisma.consultation.findMany({
        where: {
          familyId: id,
          status: { in: [$Enums.ConsultationStatus.COMPLETED, $Enums.ConsultationStatus.CANCELLED, $Enums.ConsultationStatus.DECLINED] },
        },
