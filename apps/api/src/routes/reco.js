...
router.get("/profile", async (req, res) => {
  const q = z.object({
    ...
    excludeOpen: z.string().optional(), // "1" para não trazer reservados/a-ler
  }).parse(req.query);
  ...
  try {
    ...
    if (!cp?.embedding) return res.json([]); // ainda sem perfil → devolver vazio ou fallback
    ...
    const out = rows.map((r) => {
      let status = "none";
      if (Number(r.hasOpenReading || 0) > 0) status = "reading";
      else if (Number(r.hasOpenReservation || 0) > 0) status = "reserved";
      else if (r.lastFinished) status = "finished";
      return {
        isbn: r.isbn,
        title: r.title,
