...
type RecoRow = {
  ...
  hasOpenReading: number | null;
  hasOpenReservation: number | null;
};
type RecoItem = {
  isbn: string;
  title: string;
  coverUrl: string | null;
  status: "reserved" | "reading" | "finished" | "none";
  lastFinished: Date | null;
};
...
const QuerySchema = z.object({
  ...
  excludeOpen: z.string().optional(),
});

function parseRecoQuery(q: unknown): RecoQuery {
...
function mapStatus(r: RecoRow): RecoItem["status"] {
  if (Number(r.hasOpenReading || 0) > 0) return "reading";
  if (Number(r.hasOpenReservation || 0) > 0) return "reserved";
  if (r.lastFinished) return "finished";
  return "none";
}

function toItem(r: RecoRow): RecoItem {
  return {
    isbn: r.isbn,
    title: r.title,
...
async function fetchRecoProfile(
  cid: number,
  limit: number,
  excludeOpen: boolean
): Promise<RecoItem[]> {
  const excludeOpenClause = excludeOpen
    ? Prisma.sql`AND COALESCE(h."hasOpenReading",0)=0 AND COALESCE(h."hasOpenReservation",0)=0`
    : Prisma.empty;

  const rows = await prisma.$queryRaw<RecoRow[]>`
    WITH hist AS (
      SELECT
        b."isbn",
...
const getProfileHandler: RequestHandler = async (req, res) => {
  try {
    ...
    const cpExists = await prisma.childPreference.findUnique({
      where: { childId: cid },
      select: { childId: true },
    });
    if (!cpExists) {
