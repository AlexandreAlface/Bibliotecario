...
async function main() {
  ...
  if (cmd === "by-id") {
    const id = Number(a1);
    if (!Number.isFinite(id)) throw new Error("Falta o id numérico.");
  ...
  if (cmd === "by-child-book") {
    ...
    console.log(
      `✅ Apagadas ${r.count} leituras (childId=${childId}, isbn=${isbn})`
    );
    return;
  }
  ...
  if (cmd === "open-for-child") {
    const childId = Number(a1);
    if (!Number.isFinite(childId)) throw new Error("Falta childId numérico.");
    const r = await prisma.reading.deleteMany({
      where: { childId, finishedAt: null },
    });
    console.log(
      `✅ Apagadas ${r.count} sessões em aberto (childId=${childId})`
    );
    return;
  }
  ...
  if (cmd === "truncate") {
    ...
    console.log(
      '🧹 TRUNCATE em "Reading" concluído (RESTART IDENTITY CASCADE).'
    );
    return;
  }
