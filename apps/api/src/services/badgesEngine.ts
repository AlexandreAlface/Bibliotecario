...
import { PrismaClient, MicroContentType } from "@prisma/client";
...
const diffDays = (a: Date, b: Date) => Math.floor((+b - +a) / DAY_MS);
...
function buildGenresFrom(category?: string | null, genres?: string[] | null) {
...
async function checkCurioso(childId: number) {
  ...
  const grouped = await prisma.microInteraction.groupBy({
    ...
    where: {
      userId: { in: famIds },
      microContent: { type: MicroContentType.FACTO, isPublished: true },
    },
  });
...
async function checkLeitorFrequente(childId: number) {
  ...
  const days = Array.from(new Set(rows.map((r) => toYmd(r.finishedAt!)))).map(
  ...
  for (let j = 0; j < days.length; j++) {
    while (diffDays(days[i], days[j]) > 6) i++;
    if (j - i + 1 >= 3) return true; // 3 dias dentro de 7
  }
  return false;
}
...
async function checkIlustradorDePalavras(childId: number) {
  ...
  const isIllustrated = (cat?: string | null, genres?: string[] | null) => {
  ...
  const count = rows.filter((r) =>
    isIllustrated(r.book?.category, r.book?.genres)
  ).length;
  return count >= 5;
}
...
async function checkBoaNoiteLivro(childId: number) {
  ...
  const count = rows.filter((r) =>
    hit(r.book?.title, r.book?.category, r.book?.genres)
  ).length;
  return count >= 3;
}
...
async function checkDescobridorCuriosidades(childId: number) {
  ...
  const grouped = await prisma.microInteraction.groupBy({
    ...
    where: {
      userId: { in: famIds },
      microContent: { type: MicroContentType.DICA, isPublished: true },
    },
  });
...
async function checkAventureiroLiterario(childId: number) {
  ...
  for (const r of rows) {
    buildGenresFrom(r.book?.category, r.book?.genres).forEach((g) =>
      bag.add(g)
    );
  }
  const has = (k: string) => Array.from(bag).some((c) => c.includes(k));
...
async function checkContadorDeHistorias(childId: number) {
  ...
  const long = rows.filter(
    (r) => (r.comment?.trim().length ?? 0) >= 200
  ).length;
  return long >= 3;
}

/** 16–19: dependem de dados não existentes no schema → ficam desativados. */
async function checkExploradorGlobal(_childId: number) {
  return false;
}
async function checkMetaAtingida(_childId: number) {
  return false;
}
async function checkEmbaixadorDaLeitura(_childId: number) {
  return false;
}
async function checkGuardiaoDaBiblioteca(_childId: number) {
  return false;
}
...
export async function checkAndAwardAllForChild(childId: number) {
  ...
  if (!passedNames.length) return { newlyAwarded: 0 };
  ...
  if (!badges.length) return { newlyAwarded: 0 };
